...
- **DOCX / PPTX 書き出し（デスクトップ版、`desktop/export.js`）:** AIにファイルを作らせず、回答の Markdown を `docx` と `pptxgenjs` で組み立てる。＋メニューの「このブランチを Word 文書に / スライドに」（回答ごとのボタンは利用者の指示で廃止）。`/api/export` が保存ダイアログを出し、`/api/open` で開く。表は実寸幅（dxa）で指定する（% 幅は QuickLook で潰れる）。
