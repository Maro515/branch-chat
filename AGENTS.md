...
- **DOCX / PPTX 書き出し（デスクトップ版、`desktop/export.js`）:** AIにファイルを作らせず、回答の Markdown を `docx` と `pptxgenjs` で組み立てる。回答ごとの「📄 DOCX」「📊 PPTX」と、＋メニューの「このブランチを Word 文書に / スライドに」。`/api/export` が保存ダイアログを出し、`/api/open` で開く。表は実寸幅（dxa）で指定する（% 幅は QuickLook で潰れる）。
