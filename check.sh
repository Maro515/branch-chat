...
# 1) index.html のインラインJS構文  2) 禁止パターン  3) bridge.py / desktop の構文と安全フラグ
cd "$(dirname "$0")" || exit 1
fail=0
...
no(r'(?<![A-Za-z])confirm\(|(?<![A-Za-z])prompt\(',idx,'index.html に confirm()/prompt() が無い（askConfirm/askPrompt を使う）')
no(r'sk-ant-[A-Za-z0-9]',idx,'APIキーらしき文字列が無い')
no(r'ls(Get|Set)=[^;]*\{try\{(return )?ls(Get|Set)\(',idx,'保存ヘルパーが自分自身を呼んでいない（過去に一括置換で壊れた）')
